...
Despite the fact there are many features which visually distinguish the different meta names, dimensionality reduction output didn't show good separation. We standardized the values and performed both PCA and t-SNE approaches on all numerical features to see if the clusters appear there. Especially after unsuccessful clustering it would be good to see it here. PCA showed a small separation but t-SNE not. The result of PCA you can see on figure \ref{fig:pcatsne}
 
\section*{Conclusion of the chapter}
In this chapter we performed exploratory data analysis by visualizing the features and grouping by the meta names. As we see, there features which separate well the meta names (the tag, font size, digits share, text length, etc) and which not (x and y coordinates, font family, color). We performed deminsionality reduction with PCA and see a small separation on clusters whereas k-means clustering on X and Y coordinates didn't perform well.  